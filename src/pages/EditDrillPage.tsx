...
export default function EditDrillPage() {
  ...
  useEffect(() => {
    if (drillId) {
      ...
      if (drill) {
        setName(drill.name);
        setShortDescription(drill.shortDescription);
        setDescription(drill.description);
        setWhatToLookFor(drill.whatToLookFor);
        setCategory(drill.category);
        setExperienceLevel(drill.experienceLevel);
        setDuration(drill.duration);
        setEquipment(drill.equipment);
        setResources(drill.resources);
        setImageUrl(drill.imageUrl || '');
      } else {
        toast.error('Drill not found');
        navigate('/drills');
      }
    }
  }, [drillId, drills, navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await useStore.getState().updateDrill(drillId!, {
        name,
        shortDescription,
        description,
        whatToLookFor,
    ...
    <PageLayout className="bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-800">Edit Drill</h1>
        </div>
