...
const validateTeamManagerPermission = async (
  teamId: string, 
  userId: string
): Promise<boolean> => {
  try {
...
export const handleJoinRequest = functions.https.onCall(async (data, context) => {
  ...
  if (!data.teamId || !data.requestId || !data.status) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Team ID, request ID, and status are required'
    );
  }
  ...
  try {
    ...
    // Find the request
    const request = teamData.joinRequests.find((req: any) => req.id === requestId);
    ...
    const updatedRequests = teamData.joinRequests.map((req: any) =>
      req.id === requestId ? { ...req, status, updatedAt: firestore.Timestamp.now() } : req
    );
    ...
    return {
      ...
      updatedTeam: {
        ...
        joinRequests: teamData.joinRequests.map((request: any) => ({
          ...request,
          ...
          createdAt: request.createdAt instanceof admin.firestore.Timestamp 
            ...
            : request.createdAt,
          updatedAt: request.updatedAt instanceof admin.firestore.Timestamp 
        ...
        createdAt: teamData.createdAt instanceof admin.firestore.Timestamp 
          ...
          : teamData.createdAt,
        updatedAt: teamData.updatedAt instanceof admin.firestore.Timestamp 
...
export const addPlayerToTeam = functions.https.onCall(async (data, context) => {
  // Require authentication
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated', 
  ...
  if (!data.teamId || !data.playerData) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Team ID and player data are required'
    );
  }
  ...
  try {
    ...
    return await admin.firestore().runTransaction(async (transaction) => {
      ...
      const currentPlayers = teamData.players || [];
      const playerExists = currentPlayers.some((p: any) => p.id === playerId);
...
export const assignParentToPlayer = functions.https.onCall(async (data, context) => {
  // Require authentication
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated', 
  ...
  if (!data.teamId || !data.parentId || !data.playerId) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Team ID, parent ID, and player ID are required'
    );
  }
  ...
  try {
    ...
    if (!resolvedParentName) {
      ...
      if (parentDoc.exists) {
        ...
        resolvedParentName = parentData?.displayName || 'Unknown';
      }
    }
      ...
      const updatedPlayers = currentPlayers.map((player: any) => {
        if (player.id === playerId) {
          ...
          if (!parentAlreadyAssigned) {
            return {
              ...player,
              parents: [...playerParents, parentEntry]
            };
          }
        }
        return player;
...
export const validateTeamInvite = functions.https.onCall(async (data, context) => {
  // Validate required parameters
  if (!data.teamId || !data.inviteHash) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Team ID and invite hash are required'
    );
  }
...
export const removePlayerFromTeam = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      'internal',
      error instanceof Error ? error.message : 'Error removing player'
    );
  }
});
 ...
 */
export const updatePlayerDetails = functions.https.onCall(async (data, context) => {
  ...
  if (!teamId || typeof teamId !== 'string') {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'The function must be called with a valid team ID.'
    );
  }
  ...
  if (!playerId || typeof playerId !== 'string') {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'The function must be called with a valid player ID.'
    );
  }
  ...
  if (!playerData || typeof playerData !== 'object') {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'The function must be called with player data.'
    );
  }
  ...
  try {
    ...
    const updatedPlayer = await admin.firestore().runTransaction(async (transaction) => {
      ...
      if (playerIndex === -1) {
        throw new functions.https.HttpsError(
          'not-found',
          'The specified player does not exist in this team.'
        );
      }
 ...
 * Requires authentication and team manager permission
 */
export const removeCoachFromTeam = functions.https.onCall(async (data, context) => {
  // Require authentication
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated', 
  ...
  if (!data.teamId || !data.coachId) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Team ID and coach ID are required'
    );
  }
 ...
 * Requires authentication and team manager permission
 */
export const unlinkParentFromPlayer = functions.https.onCall(async (data, context) => {
  // Require authentication
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated', 
  ...
  if (!data.teamId || !data.playerId || !data.parentId) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Team ID, player ID, and parent ID are required'
    );
  }
  ...
  try {
    ...
    return await admin.firestore().runTransaction(async (transaction) => {
      ...
      // Find the player
      const currentPlayers = teamData.players || [];
      const playerIndex = currentPlayers.findIndex((p: any) => p.id === playerId);
      ...
      const updatedParents = currentParents.filter((p: any) => p.id !== parentId);
