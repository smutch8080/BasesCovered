...
webpush.setVapidDetails(
  'mailto:steve.mutch@gmail.com', // Change this to your actual email
  vapidKeys.publicKey,
 ...
 */
export const sendPushToUser = functions.https.onCall(
  async (data, context) => {
    ...
    try {
      ...
      const notificationOptions = {
        ...
        apns: {
          ...
          fcmOptions: {
            imageUrl: options.apns?.fcmOptions?.imageUrl || null,
            ...(options.apns?.fcmOptions || {})
          },
          ...(options.apns || {})
        },
        // FCM options
 ...
 */
export const sendPushToTeam = functions.https.onCall(
  async (data, context) => {
    try {
      // Check authentication
      if (!context.auth) {
 ...
 */
export const onNewPracticePlan = functions.firestore
